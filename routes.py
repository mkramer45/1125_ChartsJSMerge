...
def Nahant():
	conn = sqlite3.connect('SurfSend.db')
	#function from app.py, responsible for populating graph
	...
	print(mylist)
	print(values)
	#original default code which populates our table on Nahant.html
	cursor = conn.cursor()
	...
	info1 = [dict(SwellSize=row[0], SwellInterval=row[1], WindSpeed=row[2], WindDescription=row[3], AirTemperature=row[4], Beach=row[5], Day=row[6], Tiempo=row[7], St8=row[8]) for row in cursor.fetchall()]
	cursor.close()
	return render_template('Nahant.html', selected='submit', info1=info1, values=values, mylist=mylist)
